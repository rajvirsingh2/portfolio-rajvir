 ...
 *
 * Position/tilt/eyes are driven by motion values (no React re-render
 * per mousemove) so the blob keeps up with fast cursor movement.
 */
export function MouseGlow() {
  ...
  // Snappy springs — high stiffness, low mass, so the pet keeps up
  const sx = useSpring(mx, { stiffness: 600, damping: 32, mass: 0.12 });
  const sy = useSpring(my, { stiffness: 600, damping: 32, mass: 0.12 });
  const sTilt = useSpring(tilt, { stiffness: 400, damping: 24, mass: 0.15 });
  const sEyeX = useSpring(eyeX, { stiffness: 700, damping: 35, mass: 0.1 });
  const sEyeY = useSpring(eyeY, { stiffness: 700, damping: 35, mass: 0.1 });
  const pupilX = useTransform(sEyeX, (v) => v * 0.4);
  const pupilY = useTransform(sEyeY, (v) => v * 0.4);

  // Label pill trails the blob
  const pillX = useTransform(sx, (v) => v + 36);
  const pillY = useTransform(sy, (v) => v + 8);
  ...
  useEffect(() => {
    ...
    const move = (e: MouseEvent) => {
      // First event after mount: no previous point, so no delta — prevents
      // a phantom "shake" from the sentinel position
      if (!last) {
        last = { x: e.clientX, y: e.clientY };
        mx.jump(e.clientX + 14);
        my.jump(e.clientY + 14);
        return;
      }
      ...
      eyeX.set(clamp(dx * 0.4, 3));
      eyeY.set(clamp(dy * 0.4, 3));
  ...
  return (
    <>
      <motion.div
        ...
        style={{ x: sx, y: sy }}
      >
        <motion.div style={{ rotate: tripping ? 0 : sTilt }}>
          <motion.div
            animate={
              tripping
                ? {
                ...
                : { rotate: 0, scale: isHovering ? 1.35 : 1 }
            }
            transition={
              tripping
          ...
          >
            ...
            <div className={`relative w-7 h-7 rounded-full bg-gradient-to-br from-emerald-400 to-blue-500 shadow-[0_2px_12px_rgba(52,211,153,0.45)] flex flex-col items-center justify-center ${tripping ? "animate-trip" : ""}`}>
              {/* Eyes */}
              <motion.div className="flex gap-[5px]" style={{ x: sEyeX, y: sEyeY }}>
                {[0, 1].map((eye) => (
